...
const ExpiringSoonAlert: React.FC<ExpiringSoonAlertProps> = ({ count }) => {
  return (
    <div className={`p-6 rounded-xl shadow-lg border-l-4 ${
      ...
      <p className={`text-4xl font-extrabold mt-2 ${count > 0 ? 'text-yellow-600 dark:text-yellow-400' : 'text-green-600 dark:text-green-400'}`}>
        {count}
      </p>
      {count > 0 && (
        <Link to="/dashboard/inventory" className="text-xs font-medium text-yellow-500 hover:text-yellow-700 mt-2 block">
          Review Expiry Dates
        </Link>
      )}
    </div>
