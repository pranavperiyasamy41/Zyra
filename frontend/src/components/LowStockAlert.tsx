...
const LowStockAlert: React.FC<LowStockAlertProps> = ({ count }) => {
  return (
    <div className={`p-6 rounded-xl shadow-lg border-l-4 ${
      ...
      <p className={`text-4xl font-extrabold mt-2 ${count > 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}>
        {count}
      </p>
      {count > 0 && (
        <Link to="/dashboard/inventory" className="text-xs font-medium text-red-500 hover:text-red-700 mt-2 block">
